...
async def test_get_rephrase_history_by_time(neural_client, mock_db, mock_ollama_client):
    ...
    mock_db.get_history_by_time.assert_awaited_once_with((dto.start, dto.end))


@pytest.mark.asyncio
async def test_get_rephrase_history_by_size(neural_client, mock_db, mock_ollama_client):
    dto = HistoryBySizeDTO(size=3)
    ...
    mock_db.get_history_by_size.assert_called_once_with(dto.size)
